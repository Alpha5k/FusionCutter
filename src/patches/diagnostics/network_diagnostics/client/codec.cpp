...
// Selects one authoritative Soldier pose from each decoded update for paired pose analysis.
void before_soldier_read(void*, const soldier_state_pipeline::ReadContext& read,
                         soldier_state_pipeline::ObserverState& state) noexcept {
    if (read.soldier != nullptr && select_pose(gAuthoritySampler)) {
        state.first = trace::read_stamp().timestamp;
        state.second = 1;
    }
}

// Captures the authoritative Soldier transform installed by an update decode.
void after_soldier_read(void*, const soldier_state_pipeline::ReadContext& read,
    ...
    gAuthoritySampler.record = {
        .object = pointer_id(read.soldier),
        .related = pointer_id(read.stream),
        .stage = 1,
        .player = read_native_field<std::int32_t>(read.soldier, 0x314),
        .mode = 0,
        .duration = trace::read_stamp().timestamp - state.first,
    };
    std::memcpy(gAuthoritySampler.record.matrix.data(), static_cast<const std::byte*>(read.soldier) + 0xF0,
...
soldier_state_pipeline::ObserverCallbacks make_soldier_state_observer(NetworkDiagnostics& diagnostics) noexcept {
    return {
        .context = &diagnostics,
